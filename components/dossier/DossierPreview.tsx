...
// the full 100+ page personalized "Dossier 2.0" book as a flip-through
// double-page-spread magazine (see DossierBook): a curiosity-driving ~10%
// of pages (cover, contents, chapter-opening legendary quotes) render for
// real via pdf.js, everything else shows a locked page you can still flip
// past. Paying (currently a mock gateway, see lib/payment/gateway) unlocks
// every page for in-browser reading and download.
//
// The dossier's content — name, archetype, realm, traits — comes entirely
// from the participant's already-decided Giantverse identity. Nothing here
// changes or re-derives that identity.
...
export function DossierPreview() {
  ...
  const { birthName, legacyName, firstName, archetype, order, guidingPromise, scores } = useSessionStore();
  ...
  const generate = useCallback(async () => {
    if (!birthName || !legacyName || !archetype || !order || !guidingPromise) return;
    setStage("generating");
    setError(null);
    try {
      const res = await fetch("/api/dossier/generate", {
        ...
        body: JSON.stringify({
          realName: firstName, birthName, legacyName, archetypeId: archetype,
          order, guidingPromise, scores,
        }),
      });
      if (!res.ok) throw new Error("generation failed");

      const pages = Number(res.headers.get("x-dossier-pages") || "0");
      const quoteStr = res.headers.get("x-dossier-quote-pages") || "";
      const quotePages = quoteStr.split(",").filter(Boolean).map(Number);
      ...
      setError("Your dossier couldn't be generated — please try again.");
      setStage("error");
    }
  }, [birthName, legacyName, firstName, archetype, order, guidingPromise, scores]);
  ...
  // effect or anything here. Memoizing this as a promise (rather than
  // populating a ref from a separate effect) avoids a race where the
  // child's effect asks for a page before a sibling effect here has had a
  // chance to start loading the document.
  ...
  function handleDownload() {
    if (!pdfBytes || !legacyName) return;
    const url = URL.createObjectURL(new Blob([pdfBytes.slice()], { type: "application/pdf" }));
    const a = document.createElement("a");
    a.href = url;
    a.download = `${legacyName.replace(/\s+/g, "-").toLowerCase()}-dossier.pdf`;
    a.click();
    ...
    <div className="legacy-container container2" style={{ minHeight: "100vh" }}>
      ...
      <div className="container-fluid" style={{ paddingBottom: 48 }}>
        <div className="content" style={{ maxWidth: 820, margin: "0 auto", paddingTop: 40 }}>
          <p className="f-12 txt-center txt-thm-clr-50-2 txt-upp letter-spacing2 mb-1">Your Giantverse Dossier</p>
          <h1 className="txt-center h2 fw-600 mb-2" style={{ color: "#EFE9DA", fontFamily: "Georgia, serif" }}>
            {legacyName}'s Character Genesis Dossier
          </h1>
          <p className="mxw-450 m-auto txt-center f-13 txt-thm-clr-70-2 line-ht-20 mb-4">
            A {totalPages || 100}+ page premium collector's-edition book, generated from your Giantverse identity —
            legendary-creator master studies, a full character-design blueprint, and a personalized Hero Journey.
            Flip through it below — locked pages open the way in.
          </p>
          ...
          {stage === "generating" && (
            <div className="txt-center" style={{ paddingTop: 60, paddingBottom: 60 }}>
              ...
              <p className="f-12 txt-thm-clr-70-2">Binding your dossier — this takes a few seconds…</p>
            </div>
          )}
          ...
          {stage === "error" && (
            <div className="txt-center">
              ...
              <button type="button" className="btn bdr-rds2" onClick={generate}>Try Again</button>
            </div>
          )}
